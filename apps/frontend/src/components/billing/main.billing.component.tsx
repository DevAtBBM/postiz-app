      ...
      (
        await (
          await fetch('/billing/prorate', {
            method: 'POST',
            body: JSON.stringify({ period, billing: pack }),
          })
        ).json()
      ).price
    );
...
export const Features: FC<{
  ...
  const features = useMemo(() => {
    ...
    const list = [];
    list.push(`${currentPricing.channel} ${currentPricing.channel === 1 ? 'channel' : 'channels'}`);
    list.push(
      `${currentPricing.posts_per_month > 10000 ? 'Unlimited' : currentPricing.posts_per_month} posts per month`
    );
    if (currentPricing.team_members) list.push('Unlimited team members');
...
export const MainBillingComponent: FC<{ sub?: Subscription }> = (props) => {
  const { sub } = props;
  ...
  useEffect(() => {
    if (initialChannels !== sub?.totalChannels) setInitialChannels(sub?.totalChannels || 1);
  ...
  const handleRazorpaySubscriptionModal = useCallback(async (subscriptionData: any) => {
    ...
    const options = {
      ...
      handler: async function () {
        toast.show('Payment successful! Your subscription is being activated and may take a few minutes to reflect.');
        setLoading(false);
  ...
  const currentPackage = useMemo(() => {
    ...
    if (period === 'YEARLY' && monthlyOrYearly === 'off') return '';
    if (period === 'MONTHLY' && monthlyOrYearly === 'on') return '';
  ...
  const moveToCheckout = useCallback(
    (billing: 'STANDARD' | 'PRO' | 'ULTIMATE' | 'FREE') => async () => {
      ...
      if (billing === 'FREE') {
        if (subscription?.cancelAt) {
          ...
          if (confirm) {
            ...
            const result = await (
              await fetch('/billing/cancel', {
                method: 'POST',
                body: JSON.stringify({ feedback: 'Reactivating subscription' }),
                headers: { 'Content-Type': 'application/json' },
              })
            ).json();
            setSubscription((subs) => ({ ...subs!, cancelAt: result.cancel_at }));
          ...
          if (
            ...
            toast.show('Subscription set to cancelled successfully');
            setLoading(false);
          }
        }
        return;
      }
      if (messages.length && !(await deleteDialog(messages.join(', '), 'Yes, continue'))) return;
      setLoading(true);
      ...
      const { url, portal, provider: responseProvider } = response;
      ...
      if (responseProvider === 'razorpay' && response.subscriptionId) {
        await handleRazorpaySubscriptionModal(response);
        return;
      }
      if (portal) {
        if (await deleteDialog('We could not process your payment, please update your payment method', 'Update', 'Payment Method Required')) {
          window.open(portal);
        }
      } else {
        setPeriod(monthlyOrYearly === 'on' ? 'YEARLY' : 'MONTHLY');
        setSubscription((subs) => ({ ...subs!, subscriptionTier: billing, cancelAt: null }));
        mutate('/user/self', { ...user, tier: billing }, { revalidate: false });
        toast.show('Subscription updated successfully');
      }
      setLoading(false);
    },
    [monthlyOrYearly, subscription, user, utm, selectedProvider, dub]
  );
    ...
    <div className="flex flex-col gap-[16px]">
        ...
        <div className="flex gap-4 p-4 bg-sixth border border-customColor6 rounded-lg">
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4"
            />
            <label htmlFor="paypal" className="cursor-pointer text-sm font-medium">PayPal</label>
          </div>
          <div className="flex items-center gap-2">
            <input
              type="radio"
              ...
              className="w-4 h-4"
            />
            <label htmlFor="razorpay" className="cursor-pointer text-sm font-medium">Razorpay (INR)</label>
          </div>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={clsx(
                  ...
                  isCurrentPlan ? 'border-green-500 shadow-lg bg-green-50/10' : 'border-customColor6'
                )}
              >
                ...
                <div className="text-[14px] flex gap-[10px]">
                  {isCurrentPlan && subscription?.cancelAt ? (
                    ...
                    <Button
                      loading={loading}
                      disabled={
                        (!!subscription?.cancelAt && name.toUpperCase() === 'FREE') ||
                        isCurrentPlan
                      }
                      className={clsx(
                        subscription &&
                      ...
                      onClick={moveToCheckout(name.toUpperCase() as 'STANDARD' | 'PRO' | 'ULTIMATE' | 'FREE')}
                    >
                      {(() => {
                        if (isCurrentPlan) return 'Current Plan';
                        if (name.toUpperCase() === 'FREE') {
                          if (subscription?.cancelAt) {
                            return `Downgrade on ${dayjs.utc(subscription?.cancelAt).local().format('D MMM, YYYY')}`;
                          }
                          return currentPackage === 'FREE' ? 'Cancel subscription' : `Downgrade to ${name}`;
                        }
                        const hierarchy = ['FREE', 'STANDARD', 'PRO', 'ULTIMATE'];
                        const currentIdx = hierarchy.indexOf(currentPackage);
                        const targetIdx = hierarchy.indexOf(name.toUpperCase());
                        if (targetIdx > currentIdx) {
                          if (currentPackage === 'FREE' && user?.allowTrial) {
                            return t('start_7_days_free_trial', 'Start 7 days free trial');
                          }
                          return 'Upgrade';
                        }
                        if (targetIdx < currentIdx) return `Downgrade to ${name}`;
                        return 'Purchase';
                      })()}
                    </Button>
                  )}
                  {subscription && !isCurrentPlan && name !== 'FREE' && (
                    <Prorate
                      period={monthlyOrYearly === 'on' ? 'YEARLY' : 'MONTHLY'}
                ...
                <Features pack={name.toUpperCase() as 'FREE' | 'STANDARD' | 'PRO' | 'ULTIMATE'} />
              </div>
            );
          })}
      </div>
      ...
      {!!subscription?.id && (
        <div className="flex justify-center mt-[20px] gap-[10px] flex-wrap">
          ...
          {isGeneral && subscription?.cancelAt && (
            <Button className="bg-green-500" loading={loading} onClick={moveToCheckout('FREE')}>
              {t('reactivate_subscription', 'Reactivate subscription')}
            </Button>
          )}
        </div>
